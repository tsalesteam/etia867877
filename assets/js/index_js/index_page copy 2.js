...
document.getElementById("ai-apply").onclick = async function(){
    ...
    let generatedHTML = sourceContainer.html().trim();
    modal.style.display = "none";
...
function updateAIGeneratedImagePaths(container) {
    ...
    container.find("img").each(function () {
        ...
        const aiFileName = nameWithoutExt + "_ai_generated.JPG";
        const relativePath = src.substring(0, src.lastIndexOf("/") + 1);

        const aiPath =
            `media/projects/${client}/${project}/${relativePath}${aiFileName}`;
...
$(document).on("click", "#confirmImage", async function () {
    ...
    formData.append("images_data", selectedImageSrc);
    formData.append("ai_generated_image", ai_generated_image);
    formData.append("client", getCookie("clientName"));
    formData.append("project", getCookie("projectName"));
    formData.append("section_id", currentSection);
    formData.append("original_image_path", oldSrc);
//  formData.append(
//     "Is_ai_generated_image_processed",
//     getCookie("Is_ai_generated_image_processed") || "no"
// );
    let list = (getCookie("ai_generated_section_images") || "")
        .split(",")
        .filter(Boolean);
    ...
    try {
        ...
        const response = await fetch("/ai/process_img/", {
            method: "POST",
            headers: {
                "X-CSRFToken": getCookie("csrftoken")
            },
        ...
        //     document.cookie = "Is_ai_generated_image_processed=yes; path=/";
        // } else {
        //     console.error("Server error:", response.status);
...
if (response.ok) {
    ...
    let aiGeneratedSection =
        adjacentBlockOfAIGeneratedSection.find(".ai-generated-wrapper");

    aiGeneratedSection.html(data.file);

    $("#ai-generated").html(data.file);

    updateAIGeneratedImagePaths($("#ai-generated"));
    updateAIGeneratedImagePaths(aiGeneratedSection);
...
function formatHTML(html) {
    ...
    function formatNode(node, level) {
        ...
        // If element contains only text, keep it on one line
        const onlyText = meaningfulChildren.every(
            child => child.nodeType === Node.TEXT_NODE
        );

        if (onlyText) {

            const text = meaningfulChildren
                .map(child => child.textContent)
                .join("")
                .replace(/\s+/g, " ")
                .trim();

            return `${indent}<${tag}${attributes}>${text}</${tag}>`;
        }
...
function getCleanHTML(container){
    ...
    clone.find("*").contents().filter(function () {
        return this.nodeType === 3;
    }).each(function () {
        this.nodeValue = this.nodeValue.replace(/\s+/g, " ").trim();
    });

return formatHTML(clone.html());
}
