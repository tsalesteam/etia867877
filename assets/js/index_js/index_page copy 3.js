...
.ai-apply-btn:hover{
    ...
    box-shadow:0 5px 15px rgba(167,55,41,.2);
}
...
<div class="ai-modal-content">
    ...
    <div class="ai-actions">
        ...
        <div id="original-actions" class="ai-action-group">
            <button id="generate-content-update-image" class="ai-btn ai-primary-btn">
                Generate Content & Update Image
            </button>

            <button id="update-image" class="ai-btn ai-secondary-btn">
                Update Image
            </button>
        </div>

        <div id="generated-actions" class="ai-action-group">
            <button id="ai-generate" class="ai-btn ai-primary-btn">
                Generate Content
            </button>

            <button id="edit-images" class="ai-btn ai-secondary-btn">
                Edit Image
            </button>

            <button id="ai-apply" class="ai-btn ai-apply-btn">
                Apply Changes
            </button>
        </div>
...
document.querySelectorAll(".ai-tab").forEach(tab => {
    ...
    tab.onclick = function(){
        ...
        document.querySelectorAll(".ai-tab-content")
...
document.getElementById("ai-generate").onclick = async function () {
    ...
    const generatedContainer = document.getElementById("ai-generated");

    const htmlContent = generatedContainer.innerHTML;
    ...
    try {
        ...
        document.getElementById("ai-generated").innerHTML =
            data.generated;
...
$(document).on(
    ...
    function(){
        ...
        const originalHTML =
            document.getElementById("ai-original").innerHTML;

        document.getElementById("ai-generated").innerHTML =
            originalHTML;

        generatedText = originalHTML;
...
function generateContent(sectionId){
    ...
    if(!originalSectionMap[sectionId]){
        ...
        $(cloned).find(".generate-btn").remove();
        $(cloned).find(".ai-toggle").remove();
        $(cloned).find(".ai-generated-wrapper").remove();
    ...
    document.getElementById("ai-generated").innerHTML = originalHTML;
    generatedText = originalHTML;
...
$("#edit-images").on("click", function () {
    ...
    /*
     * Edit Image should ALWAYS work on
     * the AI Generated tab.
     */
    const container = $("#ai-generated");

    if (!container.length) {
        console.warn("AI Generated container not found");
        return;
    }

    /*
     * Make sure AI Generated tab is active.
     */
    ...
    const generatedTab = document.querySelector(
        '[data-tab="generated"]'
    );
    ...
    /*
     * Store the current background images before
    ...
    /*
     * Reset previous image-edit data.
     */
    allImageElements = [];
    ...
    container.find("img").each(function () {
        ...
        /*
         * Save image dimensions before editing.
         * This prevents the image size from changing
         * after selecting a new image.
         */
        const rect = img.getBoundingClientRect();

        imageMetaMap.set(img, {
            width: rect.width,
            height: rect.height,
            aspectRatio:
                rect.height
        ...
        /*
         * Visual indication that the image
         * can be clicked.
         */
        $(img).css({
        ...
        /*
         * Remove any previous image-edit handler
    ...
    console.log(
        "Image edit mode enabled:",
        allImageElements.length,
        "images found"
    );
});
...
$(document).on("click", "#confirmImage", async function () {
    ...
    if(!selectedImageSrc || !currentEditingImg){
        return;
    }

    const $el = $(currentEditingImg);

    let item = allImageElements.find(
        i => i.el === currentEditingImg
    );
    ...
    if(item.type === "img"){
        ...
        const bg =
            window.getComputedStyle(currentEditingImg)
                .backgroundImage;
    ...
    try{
        ...
        selectedImageSrc =
            await toDataURL(selectedImageSrc);
    ...
    const dims =
        imageMetaMap.get(currentEditingImg) || {};
    ...
    /*
     * Keep image change inside AI Generated tab.
     * Nothing is sent to backend here.
     */
    resetImageEditMode($("#ai-generated"));
...
function updateActionButtons(tab){
    ...
    const originalActions =
        document.getElementById("original-actions");

    const generatedActions =
        document.getElementById("generated-actions");
    ...
    if(modal.dataset.mode === "image-only"){

        generateContentBtn.style.display = "none";

    }else{

        generateContentBtn.style.display = "inline-block";
    }
}
